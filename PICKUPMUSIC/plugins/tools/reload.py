...
from PICKUPMUSIC.utils.database import get_assistant, get_authuser_names, get_cmode
from PICKUPMUSIC.utils.decorators import ActualAdminCB, AdminActual, language
from PICKUPMUSIC.utils.formatters import alpha_to_int, get_readable_time
...
async def restartbot(client, message: Message, _):
    ...
    try:
        db[message.chat.id] = []
        await Line.stop_stream_force(message.chat.id)
    except:
        pass
    ...
    if chat_id:
        ...
        try:
            db[chat_id] = []
            await Line.stop_stream_force(chat_id)
        except:
            pass
